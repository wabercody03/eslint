...
function normalizeToCwd(cwd, basePath, pattern) {
...
/**
 * Globs a directory for the given patterns while honoring unignored patterns.
 * @param {string[]} filePatterns The glob patterns to find. 
 * @param {object} options The globby options to use.
 * @param {string} options.cwd The curent working directory to glob from.
 * @param {FlatConfigArray} options.configs The configs to test against.
 * @returns {Promise<string[]>} The filenames matching the globs.
 */
async function globbyWithUnignores(filePatterns, {
    cwd,
    configs
}) {
    ...
    /*
     * Split the ignore patterns into two groups: the unignore patterns
     * (that begin with !) and the regular ignore patterns. The unignore
     * patterns will have the ! removed for the next step. Both types of
     * patterns are then calculated relative to the current working
     * directory so they'll work correct. Note that the cwd and the
     * base path of the configs may be different, but the cwd is always
     * either a descendant of the base path or equal to it.
     */
    // configs.ignores.forEach(pattern => {
    ...
    //         unignorePatterns.push(
    //             normalizeToCwd(cwd, configs.basePath, pattern.slice(1))
    //         );
    //     } else {
    //         ignorePatterns.push(
    //             normalizeToCwd(cwd, configs.basePath, pattern)
    //         );
    //     }
    // });
const ignores = configs.ignores.filter(matcher => typeof matcher === "string");
    const unignorePatterns = ignores
        .filter(pattern => pattern.startsWith("!"))
        .map(pattern => pattern.slice(1));
    const ignorePatterns = ignores.filter(pattern => !pattern.startsWith("!"));

    /*
     * Because globby (via fast-glob) does not support unignore patterns, we
     * need to take a few steps to get the same results:
     * 
     * 1. Remove all unignores from the ignores list and do a regular globby
     *    call to get the results. This produces the same result as not
     *    removing the unignores right now, because fast-glob doesn't use them,
     *    but at least we won't need to update this code immediately when
     *    this functionality is added to fast-glob.
     * 2. Call globby on the unignores patterns and don't pass in any ignores.
     *    This gets us the list of potential files that should be included in
     *    the results we got from step 1.
     * 3. Filter the results from step 2 using the `configs.isIgnored()`
     *    method. Because unignore patterns are only valid when seen after
     *    an ignore pattern, we can't be sure all of the results from step 2
     *    belong in the final results without this check.
     * 4. Merge the results from step 1 and step 3.
     * 
     * Also note that globby will work on empty arrays and just return an empty
     * array, so it's safe to do both globby calls in parallel.
     * 
     * fast-glob issue: https://github.com/mrmlnc/fast-glob/issues/356
     */

    const [ originalResults, unignoreResults ] = await Promise.all([
        globby(filePatterns, {
            cwd,
            absolute: true,
    ...
    return [
        ...originalResults,
        ...unignoreResults.filter(filePath => !configs.isIgnored(filePath))
    ];
...
async function findFiles({
    ...
    const globbyResults = (await globbyWithUnignores(globbyPatterns, {
        cwd,
        configs
    }));
